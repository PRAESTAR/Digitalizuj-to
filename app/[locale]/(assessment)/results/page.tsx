        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-[#6e6e73] mt-1 font-medium break-words">
              {quizTypeLabel} &middot; {new Date(assessment.completedAt || '').toLocaleDateString(intlLocale(locale))} &middot;
              {' '}{t('results.answersCount', { count: assessment.answers.length })}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-black/5 shadow-sm p-5 sm:p-8 animate-fade-in-up relative overflow-hidden">
          <div className="relative">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="min-w-0">
                <h2 className="text-xl font-bold text-[#1d1d1f]">{t('results.auditTrail')}</h2>
                <p className="text-sm text-[#6e6e73] font-medium break-words">
                  {t('results.auditTrailSubtitle')}
                </p>
              </div>
            </div>
            ...
            <div className="mb-4 p-3 rounded-xl bg-black/[0.03] border border-black/5 text-xs text-[#86868b] grid grid-cols-1 sm:grid-cols-3 gap-2">
              <div className="min-w-0 break-words">
                <span className="font-semibold text-[#1d1d1f]">{t('results.auditModelLabel')}</span>{' '}
                <span className="font-mono break-all">
                  build {process.env.NEXT_PUBLIC_MODEL_COMMIT_HASH || 'dev'}
                </span>
              </div>
              <div className="min-w-0 break-words">
                <span className="font-semibold text-[#1d1d1f]">{t('results.auditDiiLabel')}</span>{' '}
                {result.modelVersion.diiMethodologyVersion}
              </div>
              <div className="min-w-0 break-words">
                <span className="font-semibold text-[#1d1d1f]">{t('results.auditBenchmarkLabel')}</span>{' '}
            ...
            <details className="group">
              ...
              <summary className="cursor-pointer min-h-11 py-2 text-sm text-[#0068d6] hover:text-[#004a99] font-bold flex items-center gap-1.5 transition-colors break-words">
                ...
                {t('results.auditShowFull', { count: assessment.answers.length })}
              </summary>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-black/[0.03] text-[#86868b] font-bold">
                        <th className="text-left py-3 px-3 whitespace-nowrap">{t('results.auditColQuestion')}</th>
                        <th className="text-left py-3 px-3 whitespace-nowrap">{t('results.auditColAnswer')}</th>
                        <th className="text-right py-3 px-3 whitespace-nowrap">{t('results.auditColScore')}</th>
                        <th className="text-center py-3 px-3 whitespace-nowrap">{t('results.auditColUnknown')}</th>
                        <th className="text-center py-3 px-3 whitespace-nowrap">{t('results.auditColSkipped')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {assessment.answers.map(a => (
                        <tr key={a.questionId} className="border-b border-black/5 hover:bg-black/[0.02] transition-colors">
                          ...
                          <td className="py-2.5 px-3 text-center">{a.isUnknown ? <span className="text-amber-600 font-bold">{t('common.yes')}</span> : <span className="text-[#86868b]/50">-</span>}</td>
                          <td className="py-2.5 px-3 text-center">{a.wasSkipped ? <span className="text-amber-600 font-bold">{t('common.yes')}</span> : <span className="text-[#86868b]/50">-</span>}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
